    ...
    <div class="fixed top-header-height right-0 w-[calc(100%-260px)] h-[calc(100vh-72px)] overflow-y-auto bg-surface p-container-padding main-transition">
        ...
        <div class="max-w-3xl">
            <nav class="flex items-center gap-2 text-on-surface-variant mb-2">
                ...
                <span class="text-label-sm font-label-sm text-primary font-bold">Edit Announcement</span>
            </nav>
            <h1 class="font-headline-md text-headline-md font-extrabold text-on-surface mb-6">Edit Announcement</h1>
            ...
            <form method="POST" action="{{ route('admin.announcements.update', $announcement->announcement_id) }}"
                  class="rounded-xl border border-outline-variant bg-surface-container-lowest shadow-sm p-6">
                @include('admin.announcements._form', ['mode' => 'edit'])
            </form>
        </div>
    </div>
@endsection
