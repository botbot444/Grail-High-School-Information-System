    ...
    <div class="fixed top-header-height right-0 w-[calc(100%-260px)] h-[calc(100vh-72px)] overflow-y-auto bg-surface p-container-padding main-transition">
        <div style="margin-bottom: 25px; display:flex; justify-content:space-between; align-items:center;">
            <div>
                ...
                <p style="color: gray;">Build a fee header and attach multiple line items.</p>
            </div>
        ...
        <div class="table-section" style="max-width: 960px;">
            <form method="POST" action="{{ route('admin.fees.store') }}" id="fee-form">
                @csrf
                    ...
                    <div>
                        ...
                        <select name="academic_year_id" required
                            ...
                            @foreach ($academicYears as $year)
                                <option value="{{ $year->year_id }}" {{ old('academic_year_id') == $year->year_id ? 'selected' : '' }}>
                                    {{ $year->label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="term_id" required
                            ...
                            @foreach ($terms as $term)
                                <option value="{{ $term->term_id }}" {{ old('term_id') == $term->term_id ? 'selected' : '' }}>
                                    {{ $term->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div style="display: flex; gap: 10px; margin-top: 25px;">
                    <button type="submit" class="btn save-btn"
                        ...
                        <i class="fa-solid fa-plus"></i> Create Fee
                    </button>
                    <a href="{{ route('admin.fees.index') }}" class="btn"
